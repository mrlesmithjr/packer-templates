...
# Default number of days since last build to determine if build required.
# If less than this number, build it.
...
def decide_action(args, username, vagrant_cloud_token):
    ...
    actions = {'change_controller': change_controller,
               'commit_manifests': commit_manifests, 'get_boxes': get_boxes,
               'repo_info': repo_info, 'upload_boxes': upload_boxes,
               'view_last_build_times': view_last_build_times,
               'view_manifests': view_manifests}

    action = actions[args.action]

    action(args, boxes, username, vagrant_cloud_token)
...
def prep_builds(repo, repo_facts, builds):
    """Prep builds directory structure"""
...
def cleanup_linked_dirs(build_dir, repo, repo_facts):
    ...
    for linked_dir in linked_dirs:
        ...
        entry = f'{build_dir}/{linked_dir}'.replace(f'{SCRIPT_DIR}/', '')
...
def get_boxes(args, boxes, vagrant_cloud_token):
    """Connect to Vagrant Cloud API and get boxes."""

    box_api_url = f'{API_URL}/box'
    logging.info('Getting all boxes')
    for root, _dirs, files in os.walk(SCRIPT_DIR):
        if 'box_info.json' in files:
            with open(os.path.join(root, 'box_info.json'), 'r') as box_info:
                data = json.load(box_info)
                box_tag = data['box_tag']

                url = f'{box_api_url}/{box_tag}'
                headers = {'Authorization': f'Bearer {vagrant_cloud_token}'}
...
def repo_info(*args):
    ...
    if args[0].action == 'repo_info':
        print(json.dumps(repo_facts))
        ...
        if 'build.sh' in files:
            with open(os.path.join(root, 'box_info.json'),
                ...
                if auto_build and build_image:
                    ...
                    os.chdir(root)
                    process = subprocess.run('./build.sh', check=False)
...
def get_box(box_info, username, vagrant_cloud_token):
    ...
    url = f"{API_URL}/box/{username}/{box_info['box_name']}"
    headers = {'Authorization': f'Bearer {vagrant_cloud_token}'}
    ...
    if response.status_code == 200:
        ...
        if current_version is not None:
            last_updated_str = json_data['current_version']['updated_at']
...
def create_box(box_info, username, vagrant_cloud_token):
    ...
    headers = {'Content-Type': 'application/json',
               'Authorization': f'Bearer {vagrant_cloud_token}'}
    payload = {
        'box':
        {
            'username': username,
...
def upload_boxes(args, boxes, username, vagrant_cloud_token):
    """Looks for upload_boxes script in each directory and then executes it."""

    get_boxes(args, boxes, vagrant_cloud_token)
    for root, _dirs, files in os.walk(SCRIPT_DIR):
        if root != SCRIPT_DIR:
            if 'box_info.json' in files:
                with open(os.path.join(root, 'box_info.json'),
...
def create_box_version(box_tag, box_version, vagrant_cloud_token):
    ...
    url = f'{API_URL}/box/{box_tag}/versions'
    headers = {'Content-Type': 'application/json',
               'Authorization': f'Bearer {vagrant_cloud_token}'}
...
def create_box_provider(box_tag, box_version, box_provider_name,
    ...
    url = f'{API_URL}/box/{box_tag}/version/{box_version}/providers'
    headers = {'Content-Type': 'application/json',
               'Authorization': f'Bearer {vagrant_cloud_token}'}
...
def upload_box(box_tag, box_path, box_version, box_provider_name,
    ...
    url = f'{API_URL}/box/{box_tag}/version/{box_version}/provider/{box_provider_name}/upload'
    payload = {}
    headers = {'Content-Type': 'application/json',
               'Authorization': f'Bearer {vagrant_cloud_token}'}
    ...
    print(
        f'Uploading box: {box_tag} version: {box_version} provider: {box_provider_name}')

    # Upload box
    response = requests.request(
        "PUT", upload_path, headers=headers, files=files)

    if response.status_code == 200:
        url = f'{API_URL}/box/{box_tag}/version/{box_version}/release'
...
def commit_manifests(*args):
    """Auto commit manifests."""

    repo, repo_facts = repo_info(*args)
    repo_path = os.getcwd()
    repo = git.Repo(repo_path)
    ...
    if commit:
        ...
        commit_msg = f'{commit_date} - Manifest Updates'
        repo.git.commit('-m', f'{commit_msg}')
        repo.git.push()


def view_manifests(*_args):
...
def view_last_build_times(*_args):
    """Find build manifests and display to stdout."""
